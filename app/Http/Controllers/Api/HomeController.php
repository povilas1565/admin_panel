...
class HomeController extends Controller
{
    ...
    public function get_category(Request $request)
    {
        try {
            ...
            $page_limit   = $this->page_limit ?? 10;

            $strapiData = $this->strapi->getCategories((int) $current_page, (int) $page_limit);

            $items          = $strapiData['data'] ?? [];
            $paginationMeta = $strapiData['meta']['pagination'] ?? null;
            ...
            foreach ($items as $item) {
                ...
                $result[] = [
                    ...
                    'image'       => isset($attrs['image']['url']) ? $attrs['image']['url'] : null,
                    'status'      => 1,
                    'created_at'  => $attrs['createdAt'] ?? null,
                    'updated_at'  => $attrs['updatedAt'] ?? null,
                ];
            }

            if (!empty($paginationMeta)) {
                $total_rows   = $paginationMeta['total']     ?? count($result);
                $total_pages  = $paginationMeta['pageCount'] ?? 1;
                $current_page = $paginationMeta['page']      ?? $current_page;
                $more_page    = $this->common->more_page($current_page, $total_pages);
                $pagination   = $this->common->pagination_array($total_rows, $total_pages, $current_page, $more_page);
            } else {
    ...
    public function get_language(Request $request)
    {
        try {
            ...
            $strapiData = $this->strapi->getLanguages((int) $current_page, (int) $page_limit);
    ...
    public function search_content(Request $request) // Type = 1- AudioBook, 2- Novel, 3- Music, 4- Artist , 5- User
    {
        try {
            ...
            $strapiData = $this->strapi->getArticles(
                ['search' => $search],
                (int) $current_page,
                (int) $page_limit
            );
            ...
            $result     = $this->mapStrapiArticleList($items);
            $pagination = $this->buildStrapiPagination($paginationMeta, (int) $current_page, count($result));
    ...
    private function mapStrapiArticleList(array $items): array
    {
        $result = [];

        foreach ($items as $item) {
            $attrs    = $item['attributes'] ?? [];
            $category = $attrs['category']['data'] ?? null;
            $language = $attrs['language']['data'] ?? null;
            $author   = $attrs['author']['data'] ?? null;

            $result[] = [
                'id'             => $item['id'],
                'title'          => $attrs['title'] ?? '',
                'short_desc'     => $attrs['short_description'] ?? '',
                'portrait_img'   => isset($attrs['cover']['url']) ? $attrs['cover']['url'] : null,
                'landscape_img'  => isset($attrs['cover_landscape']['url']) ? $attrs['cover_landscape']['url'] : null,
                'web_banner_img' => isset($attrs['web_banner']['url']) ? $attrs['web_banner']['url'] : null,
                'category_id'    => $category['id'] ?? null,
                'category_name'  => $category['attributes']['name'] ?? null,
                'language_id'    => $language['id'] ?? null,
                'language_name'  => $language['attributes']['code'] ?? null,
                'artist_id'      => $author['id'] ?? null,
                'artist_name'    => $author['attributes']['name'] ?? null,
                'full_novel'     => isset($attrs['full_novel']['url']) ? $attrs['full_novel']['url'] : null,
                'created_at'     => $attrs['createdAt'] ?? null,
                'updated_at'     => $attrs['updatedAt'] ?? null,
                'avg_rating'        => 0,
                'total_episode'     => 0,
    ...
    private function buildStrapiPagination(?array $meta, int $current_page, int $count): array
    {
        if (!empty($meta)) {
            $total_rows   = $meta['total']     ?? $count;
            $total_pages  = $meta['pageCount'] ?? 1;
            $current_page = $meta['page']      ?? $current_page;
            $more_page    = $this->common->more_page($current_page, $total_pages);
        } else {
            $total_rows   = $count;
            $total_pages  = 1;
        ...
        return $this->common->pagination_array($total_rows, $total_pages, $current_page, $more_page);
    }
